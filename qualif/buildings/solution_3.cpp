...
public:
	...
	bool ColorizeDfs(Block& block, int color) {
		static std::vector<Block*> vec;
		int bit = (color & 1);
		bool has_h5 = false;

		vec.reserve(rows_ * cols_);
		vec.clear();
		...
		// std::cerr << "- Colorize[" << (color >> 1) << "] " << steps << std::endl;
		return has_h5;
	}
	...
	bool IsValid() {
		...
		int color = 0;
		bool valid = true;
		color_bit_ = 1 - color_bit_;

		// std::cerr << "Colors: " << std::endl;
		// std::cerr << "- Color bit " << color_bit_ << std::endl;

		for (auto& b : blocks_) {
			// all blocks must be updated with color bit
			if ((b.color & 1) != color_bit_) {
				if (valid && b.height > 0) {
					++color;
					valid = ColorizeDfs(b, (color << 1) | color_bit_);
				} else {
					b.color = color_bit_;
				}
			}
